...
namespace wxp
{
	...
	X11RootWindow::X11RootWindow(int screen_number)
	{
		...
		_pixmap = XCreatePixmap(_display, screen_number, _screen->width, _screen->height, 24);
	}
	...
	{
		XImage *x11_image = XCreateImage(_display, _screen->root_visual, 24, ZPixmap, 0, image.data.pixels, image.width, image.height, 32, 4 * image.width);
